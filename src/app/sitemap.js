...
import { getValidBestatter, hasValidListingForCity } from '@/lib/records';
...
export default function sitemap() {
  const staticPages = [
    ...
    { url: `${BASE_URL}/haftungsausschluss`, lastModified: LAST_MODIFIED, changeFrequency: 'yearly', priority: 0.3 },
  ...
  const cityPages = cities
    .filter(c => hasValidListingForCity(c.slug))
    .map(c => ({
  ...
  const bestatterPages = getValidBestatter().map(b => ({
